      ...
      <Tabs defaultValue="current" className="w-full">
        ...
        <TabsContent value="calendar" className="h-[calc(100vh-200px)]">
          <Calendar
                ...
                <div className="grid grid-cols-7 gap-2 flex-grow">
                  {days.map((day) => {
                    const hasStream = scheduledStreams.some(
                      (stream) => isSameDay(new Date(stream.date), day)
                    );
                    return (
                      <Button
                        key={day.toString()}
                        ...
                        className={`h-16 flex flex-col items-center justify-center ${!isSameMonth(day, currentDate) ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-gray-100'} ${hasStream ? 'bg-primary text-primary-foreground dark:bg-primary dark:text-primary-foreground hover:bg-primary/90' : ''}`}
                      >
                        <span className="text-lg">{format(day, 'd', { locale: ar })}</span>
                        {hasStream && <div className="w-1.5 h-1.5 bg-current rounded-full mt-1" />}
                      </Button>
                    );
                  })}
                </div>
            ...
            viewMode={viewMode}  // Pass the correct viewMode
          />
        </TabsContent>
      </Tabs>
      <StreamDialog
